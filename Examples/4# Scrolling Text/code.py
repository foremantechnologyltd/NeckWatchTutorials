...
import time # This module allows sleep
...
xpos = 100 # Set the position of the text
# Enter an infinite loop to keep the display active
while True:
    xpos-=1.5 # Remove 1.5% per cycle
    text_area.x = x(xpos) # Set the text position
    if xpos < -100: # When position is less than -100 reset position to 100
        xpos = 100
    time.sleep(0.032) # Set framerate to 30 fps